...
class MaintenanceRequestRepositoryTest {
    ...
    void setUp() {
        ...
        staffRepository.deleteAll();
        {
            ...
            staffFixture = staffRepository.save(entity);
        }
    }

    private MaintenanceRequest newEntity(int seed) {
        MaintenanceRequest entity = new MaintenanceRequest();
        
        entity.setHotelId(UUID.randomUUID());
        
        entity.setRoomId(UUID.randomUUID());
        
        entity.setReportedByGuestId(UUID.randomUUID());
        
        entity.setIssueType("PLUMBING");
        
        entity.setDescription("test-security" + seed);
    ...
    void should_persist_entity_and_assign_id_when_saved() {
        MaintenanceRequest saved = repository.save(newEntity(0));
    ...
    void should_return_all_entities_when_repository_is_queried() {
        ...
        Page<MaintenanceRequest> page = repository.findAll((Specification<MaintenanceRequest>) null, PageRequest.of(0, 20));
    ...
    void should_filter_results_when_specification_is_provided() {
        repository.save(newEntity(0));
        repository.save(newEntity(1));
        Specification<MaintenanceRequest> matchNone = (root, query, cb) -> cb.disjunction();
        Page<MaintenanceRequest> page = repository.findAll(matchNone, PageRequest.of(0, 20));
